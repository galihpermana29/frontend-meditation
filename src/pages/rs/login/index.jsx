...
const RsLogIn = () => {
	...
	const handleLogin = async (value) => {
		try {
			const { data } = await rsAPI.login(value);
			localStorage.setItem('rs_token', data);
			...
			<div className="left flex-1 md:flex-[0.7] h-screen flex justify-center flex-col">
				<div className="form">
					<h1 className="font-[600] text-[20px] md:text-[24px]">
						Masuk Sebagai Rumah Sakit
					</h1>

					<Form onFinish={handleLogin}>
						<div className="text-label">RS Code</div>
						<Form.Item
							name={'rsCode'}
							rules={[
								{
									required: true,
									message: 'Masukkan RS Code',
								},
							]}>
							<Input placeholder="Masukkan RS Code" className="text-input" />
						</Form.Item>
